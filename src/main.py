...
from config_loader import FILE_LOG_LEVEL,TERMINAL_LOG_LEVEL,MODEL,CHECK_NVIDIA_GPU
...
class OllamaChat:
    def __init__(self, model="llama3", base_url="http://localhost:11434", system_prompt=None):
        """
        ...
        if system_prompt:
            ...
            logger.info(f"[INIT] System prompt configured: {len(system_prompt)} characters")
        ...
        while iteration < max_iterations:
            ...
            try:
                ...
                if tool_calls:
                    ...
                    assistant_message = message.get("content", "Nessuna risposta ricevuta")
                    self.conversation_history.append({
                        "role": "assistant",
...
def main():
    ...
    logger.info(f"[SYS] CPU:{cpu_name} Cores:{cores}/{threads} RAM:{ram_gb:.0f} GB")


    # GPU info (ONLY if Nvidia and .env requires it)
    if CHECK_NVIDIA_GPU:
        try:
            ...
            for i, line in enumerate(result.stdout.strip().splitlines()):
                name, mem = line.split(",")
                logger.info(f"[SYS] GPU {i}: {name.strip()} ({mem.strip()})")
        except Exception:
            pass
