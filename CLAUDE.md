...
- **the checker's own verdict** (`check_result: [problems] | "NO_CONFLICTS"`): the format holds — 64 replies out of 64 parsed — but the checker comments on everything, so "no NO_CONFLICTS" also covers quality remarks like "12:45 is around lunch, so it is not fasting". Fired the gate on turn 2 of four scenarios out of seven and recorded `branch_exercised=True` for a conflict that never happened.
