...
Three softer triggers were tried and each was measured out again, so do not reintroduce them:
...
- **the checker's own verdict** (`check_result: [problems]`, empty array when none): the format holds — 64 replies out of 64 parsed — but the checker comments on everything, so a non-empty `check_result` also covers quality remarks like "12:45 is around lunch, so it is not fasting". Fired the gate on turn 2 of four scenarios out of seven and recorded `branch_exercised=True` for a conflict that never happened.
- **patient-history hits** (`patient_history_warnings`, `get_patient_history_events`): one occurs on essentially every request — the threshold is permissive on purpose and the checker queries it every time — while the write still succeeds.

The last two share the property that disqualifies them: the activity still gets written. The system observed something, it did not stop anything, and whether such an observation is worth raising is the assistant's judgement.

The cost is deliberate and worth knowing when reading results: of the 71 conditional scenarios, roughly 30 have a block-shaped trigger and get their reaction clause delivered; the rest hinge on a judgement, never deliver, and record `branch_exercised=False`. Those still run and are still graded — conduct comes from the transcript — but the caregiver improvises instead of following the scripted reaction, so their end state is less determined.
